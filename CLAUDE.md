...
### 플라우드 메모 품질 필터 — 재사용 테스트 + 이층 구조 (2026-07-22, 브랜치 `claude/plaud-memo-quality-scpls1`) — ⬅️ 이번 세션 작업

지엽적인 대화 내용(행사 실무·회의록·도구 사용법)까지 전부 메모가 되던 문제를 사용자 인터뷰로 기준을 확정해 수리.
- **사용자 확정 기준**: 메모 용도는 글·책·강의·회고 전부 → 판정 기준은 **재사용 테스트**
  ("6개월 뒤 다른 글·강의·상담·책에서 다시 꺼내 쓸 수 있는가"). 남길 것 = 일반화되는 원리·방법론
  + 구체적 경험·사례. 버릴 것 = 일정·행사 실무, 회의록성 내용, 도구 사용법. 필터 방식은 **이층 구조**.
- **TRIAGE 프롬프트**(`vault_pipeline/prompts.py`): 메모 섹션에 재사용 테스트·남길 것/버릴 것 예시 명시,
  각 메모에 `판정: 확실|검토` + 판정사유 출력. 버릴 것은 아예 출력 금지. 남의 이론·강의 내용은
  재사용 가치 있으면 유지(출처를 발췌에 명기).
- **이층 라우팅**(`writers.py write_memos`): "확실"만 `제텔카스텐/1. 메모` 자동 입고,
  "검토"는 `1. 메모/_검토대기/`(frontmatter `status: 검토대기` + 판정사유)로 — 사람이 옵시디언에서
  파일 이동으로 승격/삭제. 판정 필드 없는 구형식 응답은 기존대로 자동 입고(하위 호환).
- **알림**(`telegram_notify.briefing(review_memos=)`): "🗂 메모 검토대기 N건 + _검토대기 폴더 tree 링크".
  브리핑의 메모 목록엔 확실층만 표시.
- 소급 적용은 하지 않음(새 녹음부터). 기존 수백 개 메모 청소는 원하면 별도 세션에서.
- vault_pipeline 테스트 37종 통과(신규: 검토 라우팅, 구형식 하위 호환, 검토대기 알림).
- **남은 사용자 액션**: 이 브랜치 검토/머지 후 다음 plaud-pipeline 실행부터 적용.
