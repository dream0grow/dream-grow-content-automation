...
**진행 기준 문서: `docs/ROADMAP_지식순환.md`** (§0 사용법 → §3 체크리스트 → §6 진행 로그). 상세는 전부 그 문서에 쓰고 여기는 요약만.
- 2026-09-19 전수 진단 결과: 코드 3세대 공존(루트 1세대 약 6,300줄 미참조 / `orchestrator` / `vault_pipeline` 양방향 import), 활성 카드 90개 중 제텔카스텐 참조 0, 콘텐츠→지식 환류 19건(8월)뿐, 플라우드 입력 7월 이후 0, `needs_human` 59/90, 볼트 커밋 79%가 장부 소음, 테스트 CI 미배선.
- 결정(제안): 완전 재작성 안 함. Phase 0 정리 → Phase 1 지식 배선(브리프에 내 지식 검색 주입·근거 0건 게이트·발제 전환·candidates 환류·**영상 발화 환류**(§4)·플라우드 triage 확장) → Phase 2 텔레그램 버튼 승인 → Phase 3 플라우드 입력 복구 → Phase 4 리눅스 서버.
- 코드 변경 없음(문서만). 사용자 결정 필요 항목은 문서 §5.
