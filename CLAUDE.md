...
## 드림그로우 쇼츠 편집 (2026-09-17)

- 드림그로우 채널의 촬영본 편집은 `shorts/`에서 운영한다. 시작 문서: `shorts/README.md`.
- **편집 착수 전 반드시 `shorts/EDITORIAL.md` 통독**. 9대 상시 규칙과 핵심 후킹 발굴 프로토콜:
  - ① 핵심 후킹 문구 반드시 살리기(**가장 중요** — 통념 뒤집는 문장).
  - ② 언어 도입부 제거(제목이 시각 후킹). 원본 setup 잘라내고 핵심 후킹으로 즉시 시작.
  - ③ 인용부호: 발화 인용은 큰따옴표, 강조·생각은 작은따옴표. 인용이 여러 자막이면 첫/마지막에만 여닫음.
  - ④ 자막-음성 싱크: 자막이 발화보다 먼저 뜨지 않게. 렌더 후 재청취 필수.
  - ⑤ 컷 끝음 정리: 잔음·반복 잘라내기.
  - ⑥ 컷 사이 0.3초 공백(`cut_gap`).
  - ⑦ 마지막 clip에 tail 여유 0.3~0.5초(`gap_after`).
  - ⑧ 하단 브랜드 "드림그로우".
  - ⑨ 리드 마그넷 안내 clip이 있으면 실제 자료(`lead_magnets/`)가 저장소에 존재해야 한다.
- 원본의 자기 요약과 진짜 후킹을 혼동하지 말 것. 후자는 보통 도입 5~30초에 있다(DJI_0008 v1→v2→v3 사례 참고).
- 첫 원본 `DJI_20260914072454_0008_D`, 참고 쇼츠 `https://www.youtube.com/shorts/DAeAY5ZWVEs`. v1 편집 27.1초(리드 마그넷 없는 짧은 버전), v2 편집 36.2초(리드 마그넷 안내 포함).
- 공통 디자인: `shorts/styles/growcircle_reference_v1.json` (파일명은 유지, 표시 브랜드는 job의 `brand` 필드). 영상별 원본 시각 편집표: `shorts/jobs/<원본>.v1.json`·`.v2.json`. 파일명 `.vN`은 편집 버전, 내용상 짧은 컷/리드 마그넷 포함/장문 등 다른 편집을 병렬 관리한다.
- 리드 마그넷 자료는 `lead_magnets/<주제>_v1.md`. md 초안 작성 → 사용자 수정 → PDF 변환 흐름. 관련 job의 `editorial_notes`에서 파일 경로를 명시해 영상↔자료 연결 유지.
- 산출물은 git 제외 `artifacts/shorts/<원본>/<v>/`에 MP4/SRT/설정/검증 결과로 저장한다.
- 현재는 에이전트가 내용·구간을 선택하고 렌더링을 자동 수행한다. 새 파일 무인 감시나 업로드는 아직 연결하지 않았다.
- 사용자는 계속 업그레이드할 계획이다. 피드백·개선은 `docs/shorts/CHANGELOG.md`, 첫 편 제작 기록은 `docs/shorts/DJI_0008_v1.md`에 남긴다.
- 기존 `youtube/`는 다른 채널 소유이므로 이 작업으로 변경하지 않는다.
...
### 지식 선순환 로드맵 — 코드 정리 + 제텔카스텐↔콘텐츠 배선 (2026-09-19, 브랜치 `claude/knowledge-cycle-roadmap-2026-09`) — ⬅️ 최신, 여러 세션·여러 AI가 이어가는 작업

**진행 기준 문서: `docs/ROADMAP_지식순환.md`** (§0 사용법 → §3 체크리스트 → §6 진행 로그). 근거 진단 두 벌: Claude(문서 §2) + Aside(`docs/기획/지식콘텐츠_선순환_진단_2026-09-19.md`). 상세는 전부 그 문서에 쓰고 여기는 요약만.
- 진단 요지: 코드 3세대 공존, 활성 카드 90개 중 제텔카스텐 참조 0, 콘텐츠→지식 환류 19건(8월)뿐, 플라우드 입력 7월 이후 0, 볼트 커밋 79%가 장부 소음, 테스트 CI 미배선.
  **P0**: ① 검수·발행·환류가 서로 다른 원고 버전을 봄(revision 없음) ② **매일 발제가 2026-08-24부터 Manus 401로 전부 정체(`research/queued` 27건, DISPATCH에 없는 상태)** ③ 사실 검증 관문 없음.
- 순서(제안, Aside안 채택): Phase 0 안전·정합성(로컬 체크아웃 정합·발제 정체 복구·revision·상태 분리·CI) → Phase 1 작은 완전 순환(자격 필터·근거 묶음·근거 0건 게이트·출처 게이트·candidates 환류·**영상 발화 환류**(§4)·플라우드 원문 보존) → Phase 2 텔레그램 버튼+웹 검토함 → Phase 3 코드 통합·worker·리눅스 → Phase 4 과거 자산 회수.
- 갈림길(§5-2): 진실원천을 볼트(git)로 유지(Claude안) vs Supabase로 전환(Aside안). 제안은 Phase 0~1을 볼트 기준으로 완주 후 결정.
- 코드 변경 없음(문서만). 사용자 결정 필요 항목 13개는 문서 §5.
...
- 테스트 8종 신규(`test_reels_recommend.py`). 전체 165종 통과. PR #91로 main 머지 완료(2026-08-26).
- **2차 처리(같은 날 저녁)**: 사용자가 텔레그램 일반 메시지로 보낸 요청(후보 노트로만 저장됐던 것)을
  세션에서 수동 처리 — ① 「학교 가기 싫다는 아이」 원고를 사용자 글감(선생님/친구 두 갈래 프레임)으로
  전면 재작성(윤문 A등급, 파일명 `…이유는+둘+중+하나입니다.md`로 개명, 장부 키 승계)
  ② 새 추천 3편(칭찬 역설/숙제 대신/형제갈등) 텔레그램 전송 + 장부 기록 ③ 후보 노트 `status: processed` 마감.
