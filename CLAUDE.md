...
### 쇼츠 대본 제작·보완 스킬 (2026-09-16, 브랜치 `claude/focused-hawking-tz4pkx`) — ⬅️ 이번 세션 작업

쇼츠·릴스 대본을 반복 생산하는 대화형 스킬 **`.claude/skills/dreamgrow-shorts-script`** 신설
("쇼츠 대본 만들어줘", "이 대본 보완해줘", "쇼츠 10개 뽑아줘"로 호출). 기존에 이 용도 스킬은
프로젝트·claude.ai·마켓 어디에도 없었고 재료만 흩어져 있던 것을 묶었다.
- 모드 4종: A 신규(주제→표준 릴스 5블록) / **B 보완(사용자 부분 대본 원문 보존 + 빠진 블록만 채움,
  `[원문]`/`[추가]` 표시본 → 최종본)** / C 일괄(후킹 표 승인 후 본문, 최대 5편) / D 볼트 원고 수정.
- 매번 주입: `data/hook_patterns.md` + `data/youtube_voice.md` + `prompts.HUMANIZE_RULES` + 같은 카테고리
  기존 릴스 원고 2편. 중복 검사 후 작성, 확정 전 `/im-not-strange-ai` 윤문.
- **길이 기준 실측 교정**: 낭독 초 = 글자 수 ÷ 6.5. 볼트 기존 릴스 원고는 600자≈90초인데 "약 42초"로
  표기돼 있어 기준으로 삼지 않는다. 새 대본 목표 260~325자(40~50초), 390자(60초) 상한.
- 확정본은 `05 리뷰/대기/원고_릴스_<카테고리>_<키워드>.md`(frontmatter `채널: reels`, `상태: 리뷰대기`)로
  저장 → 아침 릴스 추천·텔레그램 답장 핑퐁에 자동 편입. 예시 `examples/학습지_30분_보완.md`
  (사용자 초안 5문장 → 306자 47초 완성본).
- **남은 사용자 액션**: ① 브랜치 검토/머지 ② 대본 초안 하나 붙여 "이 대본 보완해줘"로 라이브 테스트.
