...
- newsletter: **3,000~6,000자**의 심화 콘텐츠. 스레드처럼 짧게 끊지 말고, 각 단락을
  충분히 풀어서 읽는 재미와 깊이를 주되 늘어지지 않게 한다. 아래 [필수 구성]을 모두 포함:
  1) **인트로**: 구독자에게 건네는 인사 + 독자의 상황에 공감하며 주제 도입.
     (브리프의 reaction_type에 맞춰 공감사연/실전가이드/통념뒤집기/교실관찰/QA/이슈해설 중
      도입 방식을 매번 다르게 고른다. 같은 도입을 반복하지 말 것.)
  2) **본론**: 교실 에피소드 **최소 1개**(구체적 장면·대사) + 핵심 방법 3~5단계.
     각 단계를 충분히 설명하고, 브리프의 evidence_anchors(근거)를 자연스럽게 녹인다.
     근거는 출처를 밝히고 수치 단정은 피한다('연구들이 보고한다', '경향이 있다' 식).
     'A가 아니라 B이다' 논증 구조를 적극 활용한다.
  3) **[가정에서 연습하는 법]**: 부모-자녀의 구체적 대화 예시를 실제 대사 형태로 제시.
     (예: 부모 "...", 아이 "...") 상황을 눈에 보이게 한다.
  4) **유의할 점**: 부모가 흔히 하는 실수 2~3가지와 각각의 대안.
  5) **정리**: 핵심 메시지를 압축해 다시 못박는다.
  6) **다음 호 예고**: 다음에 다룰 주제 힌트 한 줄.
