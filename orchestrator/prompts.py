...
형식 규칙:
- thread: 글 5~10개의 체인으로 구성. 1번 글은 스크롤을 멈추게 하는 훅(질문/반전),
  중간 글은 핵심 메시지와 실천 단계를 글 하나당 하나씩, 마지막 글은 정리+CTA.
  각 글은 500자 미만, 글 사이는 '---' 한 줄로 구분.
- newsletter: 인트로(독자와 연결) → 본론(교실 에피소드 + 단계별 방법) →
  [가정에서 연습하는 법] 대화 예시 → 유의할 점 → 정리 → 다음 호 예고 구조.
