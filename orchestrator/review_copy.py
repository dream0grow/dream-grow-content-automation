...
발행 원본은 어디까지나 파이프라인 카드(`vault/파이프라인/활성/원고_*_DG-....md`)다.
...
def export(card: dict, fmt: str, draft: str) -> str:
    ...
    note = (
        f"> 열람용 사본입니다 (원본 카드 {content_id}). 이 파일에서 바로 발행할 수 "
        "있어요 —\n"
        "> 본문을 고친 뒤 frontmatter 상태를 '리뷰완료'로 바꾸거나, 텔레그램 알림에 "
        "'발행해줘'라고 답장하면\n"
        "> 이 본문이 그대로 발행되고 원본 카드도 발행완료로 정리됩니다. "
        "(이 안내 인용구는 발행에서 제외돼요.)\n"
    )
